...
def analyze_papers_and_init_clusters(lines,author_name, COUNT):
    for line in lines:
        li = line.split(':')
        if li[0] == author_name:
            l = li[1].split('\t')
...
def clustering(lines,author_name, COUNT):
    ...
    papers, clusters, paper_idx_2_cluster_id, inverted_indices, author_id_set = analyze_papers_and_init_clusters(lines,
        author_name, COUNT)
...
def name_disambiguation(lines,author_name, id_gen, process_id, COUNT):

    paper_count, clusters, author_id_set, db_time, cl_time = clustering(lines,author_name, COUNT)
