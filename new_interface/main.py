...
if __name__ == '__main__':
    cursor.execute('SELECT AuthorID FROM PaperAuthorAffiliations_tmp '
                   'WHERE AuthorID LIKE "' + process_id + '%" '
                                                          'ORDER BY AuthorID DESC LIMIT 1')

    id_gen = cursor.fetchall()[0][0]
    id_gen = '1' + id_gen[1:]
    ...
    COUNT = last_count + 1
    with open('name_dis.txt') as f:
        lines = f.readlines
        for author_name in author_names_to_process:
            id_gen = name_disambiguation(lines,author_name, id_gen, process_id, COUNT)
